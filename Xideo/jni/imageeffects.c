...
#define  LOG_TAG    "libimageeffects"
...
JNIEXPORT void JNICALL Java_com_comcast_xideo_ImageEffects_blur(
		JNIEnv *env, jobject  obj, jobject bitmapIn, jobject bitmapOut, jint radius){
...
JNIEXPORT void JNICALL Java_com_comcast_xideo_ImageEffects_soften(
		JNIEnv *env, jobject  obj, jobject bitmapIn, jobject bitmapOut, jint threshold){
    LOGI("Softening bitmap...");
    ...
    int range = threshold > 0 && threshold < 50 ? threshold:25;
    int uthres = 128;
    int lthres = range;
    ...
    for (y = 0; y < h; y++) {
        for (x = 0; x < w; x++) {
        	...
        	output[x].green = g;
        	output[x].blue = b;
        	output[x].alpha = input[x].alpha;
        }
        input = (char *)input + infoIn.stride;
        output = (char *)output + infoOut.stride;
    }
